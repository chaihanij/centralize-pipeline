...
generate_semantic_version() {
  echo "::group::🔢 Generating Semantic Version"
  ...
  if ! command -v cz &> /dev/null; then
    echo "⚠️ Commitizen not found, falling back to SHA-based version"
    echo "${CURRENT_VERSION}-${SHORT_SHA}"
    echo "::endgroup::"
    return
  fi
  ...
  if [ -z "$NEXT_VERSION" ] || [ "$NEXT_VERSION" == "$CURRENT_VERSION" ] || [ $BUMP_EXIT_CODE -ne 0 ]; then
    ...
    echo "No version bump needed or error occurred, using: ${APP_VERSION}"
  else
    APP_VERSION="${NEXT_VERSION}"
    echo "New version determined: ${APP_VERSION}"
  fi
...
configure_image_tags() {
  ...
  case "$ref_name" in
    "develop")
      PRIMARY_TAG=$(generate_semantic_version)
